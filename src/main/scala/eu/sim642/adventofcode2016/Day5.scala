...
object Day5 {
  ...
  def getPassword(doorId: String, times: Int = 8): String = {
    Iterator.from(0).map(i => Hash.md5(doorId + i)).filter(_.startsWith("00000")).map(_(5)).take(times).mkString("")
  }

  def getPassword2(doorId: String): String = {
    val it = Iterator.from(0).map(i => Hash.md5(doorId + i)).filter(_.startsWith("00000")).map(s => (s(5), s(6))).filter(p => ('0' to '7').contains(p._1)).map(p => (p._1 - '0', p._2))
