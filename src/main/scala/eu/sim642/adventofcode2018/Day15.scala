...
object Day15 {
  ...
  def combatOutcomeElfWin(grid: Grid[Char], units: List[CombatUnit]): Int = {
    ...
    val attackPowers = (4 to 200).groupBy(e => math.ceil(200.0 / e).toInt).view.mapValues(_.min).values.to(LazyList).sorted // 20 and 21 both require 10 hits to kill goblin, no point in trying both
