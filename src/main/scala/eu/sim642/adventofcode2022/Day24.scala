  ...
  def fewestMinutes(input: Input): Int = {
    ...
    case class State(pos: Pos, time: Int) {
      ...
      def steps: Iterator[State] = {
        ...
        } yield State(newPos, time + 1)
      }
    }

    val graphSearch = new GraphSearch[State] with UnitNeighbors[State] {
      override val startNode: State = State(Pos(1, 0), 0)

      override def unitNeighbors(state: State): IterableOnce[State] = state.steps

      override def isTargetNode(state: State, dist: Int): Boolean = state.pos == max - Pos(1, 0) // || dist == 20 // TODO: remove dist limit
    }
