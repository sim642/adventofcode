...
class Day20Test extends AnyFunSuite with ScalaCheckPropertyChecks {
  ...
  ignore("mix") {
    // cannot test examples, because ambiguous and inconsistent
  ...
  test("Part 1 input answer") {
    assert(Part1.mixGroveCoordinates(parseFile(input)) == 4224)
  }
  ...
  test("Part 2 input answer") {
    assert(Part2.mixGroveCoordinates(parseFile(input)) == 861907680486L)
  }
}
