...
class Day14Test extends AnyFunSuite with ScalaCheckPropertyChecks {
  ...
  test("parseReactions") {
    assert(parseReactions("1 A, 2 B, 3 C => 2 D") ==
      Map("D" -> (2, Map("A" -> 1, "B" -> 2, "C" -> 3))),
    )
  }

  test("Part 1 examples") {
    val inputExpectedOre = Table(
      ("input", "expectedOre"),
      (exampleInput, 31),
      (exampleInput2, 165),
      (exampleInput3, 13312),
      (exampleInput4, 180697),
      (exampleInput5, 2210736),
    )

    forAll (inputExpectedOre) { (input, expectedOre) =>
      assert(oreForFuel(parseReactions(input)) == expectedOre)
    }
  }
